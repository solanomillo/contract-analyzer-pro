...
class AppConfig:
    ...
    modelos_chat_disponibles: List[ModeloInfo] = None
    modelos_embedding_disponibles: List[ModeloInfo] = None
    
    def __post_init__(self):
        if self.modelos_chat_disponibles is None:
            self.modelos_chat_disponibles = []
        if self.modelos_embedding_disponibles is None:
            self.modelos_embedding_disponibles = []


class ConfigService:
    """
    Servicio centralizado para manejar la configuracion.
    Evita duplicacion de codigo en toda la aplicacion.
    """
    ...
    def has_api_key(self) -> bool:
        """Verifica si hay una API key configurada (no vacia)."""
        api_key = self.get_api_key()
